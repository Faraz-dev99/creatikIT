...
export default function ContactTypeEdit() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const data: contacttypeGetDataInterface | null = await getContactTypeById(id);
      if (data) {
        setContactTypeData({ Name: data.Name, Status: data.Status });
      }
    };
    fetchData();
  }, [id]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await updateContactType(id, contactTypeData);
      if (result) {
        toast.success("Contact Type updated successfully!");
        router.push("/masters/contacttype");
      }
    } catch (error) {
          ...
          <form onSubmit={(e) => e.preventDefault()} className="w-full">
            <div className="mb-8 text-left border-b pb-4 border-gray-200">
              <h1 className="text-3xl font-extrabold text-gray-800 leading-tight tracking-tight">
                Edit <span className="text-blue-600">Contact Type</span>
              </h1>
            </div>
